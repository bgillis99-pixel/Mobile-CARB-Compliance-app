...
const ANDROID_ICON = (
  <svg className="w-4 h-4" viewBox="0 0 576 512" fill="currentColor" aria-hidden="true">
...
const App: React.FC = () => {
  ...
  const navItems = [
    { id: AppView.HOME, label: 'HUB', icon: APPLE_ICON, description: 'Return to Hub' },
    { id: AppView.ADMIN, label: 'OPS', icon: ANDROID_ICON, description: 'Admin Operations' },
    { id: AppView.ASSISTANT, label: 'AI', icon: APPLE_ICON, description: 'AI Assistant' },
    { id: AppView.GARAGE, label: 'FLEET', icon: ANDROID_ICON, description: 'Fleet Management' },
  ];
  ...
  const GlobalHeader = () => (
    <header className="pt-safe px-6 py-2 fixed top-0 left-0 right-0 glass-dark z-[100] flex flex-col items-center gap-2 pb-3 border-b border-white/5" role="banner">
      <div className="flex justify-between items-center w-full max-w-md mx-auto">
        ...
        >
          ...
          <div className="absolute -top-0.5 -right-0.5 w-2 h-2 rounded-full bg-blue-600 animate-pulse z-20 shadow-[0_0_6px_#2563eb]" aria-hidden="true"></div>
        </button>
        
        <nav className="flex flex-1 justify-end items-center gap-2 pl-4" aria-label="Main navigation">
            {navItems.map(item => (
              <button 
                key={item.id} 
  ...
  return (
    <div className="dark min-h-screen bg-carb-navy text-white overflow-x-hidden selection:bg-carb-accent">
        ...
        <main className={`flex-1 overflow-y-auto ${currentView === AppView.INTAKE || currentView === AppView.INVOICE ? 'pt-24' : 'pt-24'} pb-32`} role="main">
            <div className="px-6">
