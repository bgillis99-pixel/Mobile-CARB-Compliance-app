    ...
    <div className="flex flex-col h-[calc(100vh-180px)] bg-white rounded-2xl border-2 border-[#003366] overflow-hidden">
      <div className="bg-[#003366] text-white p-4 flex justify-between items-center">
        ...
        <div className="flex gap-2 text-xs">
          ...
          <button onClick={() => setMode('maps')} className={`px-2 py-1 rounded ${mode === 'maps' ? 'bg-[#15803d] text-white' : 'bg-blue-900 text-gray-300'}`}>Maps</button>
          <button onClick={() => setMode('thinking')} className={`px-2 py-1 rounded ${mode === 'thinking' ? 'bg-[#15803d] text-white' : 'bg-blue-900 text-gray-300'}`}>Deep Think</button>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-[#f8f9fa]">
        {messages.length === 1 && (
            <div className="flex flex-wrap gap-2 mb-4">
                <button onClick={() => setInput('Why is my vehicle blocked?')} className="bg-white border border-[#15803d] text-[#003366] px-3 py-1 rounded-full text-xs font-bold hover:bg-[#e6f4ea]">Why am I blocked?</button>
                <button onClick={() => setInput('Lost Password')} className="bg-white border border-[#15803d] text-[#003366] px-3 py-1 rounded-full text-xs font-bold hover:bg-[#e6f4ea]">Lost Password?</button>
                <button onClick={() => setInput('When is my next test deadline?')} className="bg-white border border-[#15803d] text-[#003366] px-3 py-1 rounded-full text-xs font-bold hover:bg-[#e6f4ea]">Next Test Deadline?</button>
            </div>
        )}
      ...
      <div className="p-3 bg-white border-t border-gray-200">
        <div className="relative flex items-center">
          <input
            ...
            className="w-full pl-4 pr-12 py-3 rounded-full border-2 border-[#003366] bg-white text-[#003366] placeholder:text-gray-400 focus:outline-none focus:border-[#15803d] font-medium"
          />
          <button 
            onClick={handleSend}
            disabled={loading}
