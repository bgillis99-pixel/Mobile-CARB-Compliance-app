    ...
    const handleFinalSubmit = async () => {
        ...
        try {
            await saveIntakeSubmission({
                clientName,
                timestamp: Date.now(),
                // Fix: Changed 'odo' to 'odometer' to match the IntakeSubmission type definition
    ...
    if (step === 'name') {
        return (
            <div className="max-w-md mx-auto py-10 animate-in fade-in duration-500">
                <div className="glass p-10 rounded-[3.5rem] border border-blue-500/20 space-y-10">
                    <div className="text-center space-y-2">
                        <h2 className="text-3xl font-black italic tracking-tighter uppercase">Processor Login</h2>
                        <p className="text-[9px] font-black text-blue-500 uppercase tracking-[0.4em]">Initialize Intake Flow</p>
                    </div>
                    <div className="space-y-4">
                        <input 
                            value={clientName}
                            onChange={e => setClientName(e.target.value)}
                            placeholder="OPERATOR / CLIENT NAME"
                        ...
                        <button 
                            ...
                            disabled={!clientName}
                            className="w-full py-6 bg-blue-600 text-white font-black rounded-[2rem] uppercase tracking-widest text-xs shadow-xl disabled:opacity-50"
                        >
                            Select Input Type
                        </button>
                    </div>
                ...
                {loading ? (
                    ...
                    <div className="space-y-8">
                        ...
                        <div className="flex gap-4">
                            ...
                            <button onClick={handleFinalSubmit} className="flex-[2] py-6 bg-blue-600 text-white font-black rounded-[2rem] uppercase tracking-widest text-[9px] shadow-2xl">Confirm & Save</button>
                        </div>
                    </div>
                )}
            </div>
        ...
        return (
            <div className="max-w-md mx-auto py-20 text-center space-y-10 animate-in zoom-in duration-500">
                ...
                <div className="space-y-4">
                    ...
                    <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest px-10 leading-relaxed">System metadata updated. Your fleet advisor will contact you shortly.</p>
                </div>
                <button onClick={onComplete} className="bg-white text-carb-navy px-12 py-5 rounded-[2rem] font-black uppercase text-xs tracking-widest italic active-haptic">HUB</button>
            </div>
        );
    }
